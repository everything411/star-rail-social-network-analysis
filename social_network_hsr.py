...
def load_character_data(file_path, text_map):
    ...
    for char in characters:
        # 提取角色ID
        avatar_id = char.get('AvatarID')
        if not avatar_id:
            continue
            
        # 提取角色名称哈希值
        name_data = char.get('AvatarName', {})
        if not name_data:
            continue
            
        text_hash = str(name_data.get('Hash', ''))
        if text_hash and text_hash in text_map:
            char_name = text_map[text_hash]
            character_id_to_name[avatar_id] = char_name
    
    return character_id_to_name

def extract_mentioned_characters(text, all_names):
    """从文本中提取提及的角色名"""
    mentioned = set()
...
def build_mention_graph(voice_atlas_file, text_map, character_id_to_name):
    """构建角色提及关系图，处理角色别名"""
    with open(voice_atlas_file, 'r', encoding='utf-8') as f:
    ...
    # 创建包含别名和本名的查找集合
    all_names = set(character_id_to_name.values())
    for alias, real_name in alias_map.items():
        if real_name in character_id_to_name.values():
            all_names.add(alias)
    
    for voice_entry in voice_data:
        avatar_id = voice_entry.get('AvatarID')
        if not avatar_id or avatar_id not in character_id_to_name:
            continue
            
        # 获取语音标题的哈希值
        voice_title_data = voice_entry.get('VoiceTitle', {})
        voice_title_hash = voice_title_data.get('Hash') if isinstance(voice_title_data, dict) else None
        
        if not voice_title_hash:
            continue
            
        # 获取语音标题文本
        voice_title = text_map.get(str(voice_title_hash), '')
        if not voice_title:
            continue
            
        # 获取说话者名称(使用本名)
        speaker_name = character_id_to_name[avatar_id]

        # 跳过玩家角色
        if speaker_name in [r"{NICKNAME}"]:
            continue
        
        # 提取提及的角色
        mentioned_names = extract_mentioned_characters(voice_title, all_names)
        ...
        for name in mentioned_names:
            ...
            # 确保提及的角色在正式角色列表中
            if mentioned_name in character_id_to_name.values() and mentioned_name != speaker_name:
...
<body>
    <h1 style="text-align: center;">银河角色关系网络分析报告</h1>
...
def analyze_social_network(G, output_dir="social_network_analysis"):
    ...
    plt.title("银河角色评价关系社区结构", fontsize=16, fontname='SimHei')
    plt.axis('off')
    ...
    plt.title("银河角色评价关系核心-边缘结构", fontsize=16, fontname='SimHei')
    plt.axis('off')
...
def main():
    ...
    voice_atlas_file = os.path.join(base_path, 'VoiceAtlas.json')
    avatar_file = os.path.join(base_path, 'AvatarConfig.json')  # 角色数据文件
    ...
    character_id_to_name = load_character_data(avatar_file, text_map)
    print(f"Loaded {len(character_id_to_name)} characters")
    
    print("Building mention graph...")
    graph, edge_data = build_mention_graph(voice_atlas_file, text_map, character_id_to_name)
    ...
    analysis_results = analyze_social_network(G, output_dir="star_rail_network_analysis")
    ...
    print("\n分析完成！结果已保存至 star_rail_network_analysis 目录")
